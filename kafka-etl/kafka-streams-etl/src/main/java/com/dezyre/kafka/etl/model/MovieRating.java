...
public class MovieRating implements Serializable {
    ...
    public String toString() {
        return String.format("%d##%d##%s##%s##%8.2f##%d##%s",
                this.movieId, this.userId, this.title, this.year,
                this.rating, this.timestamp, this.genres);
    }
    ...
    public MovieRating addRating(MovieRating v1, boolean takeLatest) {
        ...
        if (takeLatest) {
            this.timestamp = (this.timestamp > v1.timestamp ? this.timestamp : v1.timestamp);
        } else {
            this.timestamp = (this.timestamp < v1.timestamp ? this.timestamp : v1.timestamp);
        }
        this.userId = 0l;
